...
export default function ProductDetail() {
  ...
  useEffect(() => {
    if (selectedColor) {
      ...
      if (selectedColor.sizes && selectedColor.sizes.length > 0) {
        ...
        const sameSize = selectedColor.sizes.find(
          (s) => s.size === selectedSize?.size && s.stock > 0
        );
  ...
  if (!product)
    return (
      <div className="text-center py-20">Loading product information...</div>
    );
  ...
  const getStockMessage = () => {
    ...
    if (!selectedColor.sizes || selectedColor.sizes.length === 0)
      return "This color is temporarily out of size";
    ...
    <div className="container mx-auto py-10 px-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col gap-4 ml-auto">
          {/* Main Image */}
          <div className="w-full aspect-square rounded-lg border bg-gray-50 overflow-hidden">
            <img
              src={mainImage}
              alt={product.name}
          ...
          <div className="mt-6">
            <h3 className="text-sm font-medium text-gray-900 mb-2">
              ...
              <span className="font-bold">
                {selectedColor?.color_name || "Not selected"}
              </span>
            </h3>
            ...
            {!isProductIncomplete &&
              (!selectedColor?.sizes || selectedColor.sizes.length === 0) && (
                <p className="text-sm text-red-500 italic mt-1">
                  This color has no sizes available.
                </p>
              )}
          </div>
